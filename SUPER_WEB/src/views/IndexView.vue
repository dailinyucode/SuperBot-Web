<template>
    <div class="body" ref="scrollRef">
        <div v-if="!conversationList.length" class="explain">
            <img v-show="!modelVisible" class="logo" alt="Vue logo" src="../assets/gpt_three_big.png">
            <img v-show="modelVisible" class="logo" alt="Vue logo" src="../assets/gpt_four_big.png">
            <div class="expositoryCase" v-show="!modelVisible">欢迎使用ChatGPT 3.5智能对话</div>
            <div class="expositoryCase" v-show="modelVisible">欢迎使用ChatGPT4.0智能对话</div>
            <div class="consume">
                <el-icon>
                    <Goods/>
                </el-icon>
                <div class="consumeText" v-show="!modelVisible">每次提问消耗1次Ai币</div>
        ...
        <div v-else class="questions">
            <div v-for="(item,index) in conversationList" :key="index" class="item slide-animation">
                <div class="question">
                    <el-avatar v-show="!modelVisible" class="flexShrink" :size="28" :icon="UserFilled"
                ...
                <div class="answer">
                    ...
                    <div v-if="item.assistant" style="flex: 1">
                        ...
                        <div v-if="!item.isError" class="operation">
                            ...
                            <div @click="onCollection(item,index)" class="operationItem"
                                ...
                                <div class="operationExplain"> {{ item.isCollection ? '取消收藏' : '收藏' }}</div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="footer">
            <div class="footer-bar">
                <el-input @keyup.enter="onSubmit" v-model="input" :placeholder="aiPrompt" :disabled="aiLoading">
                    <template #prepend>
                        <el-select v-model="model" placeholder="Ai模型" style="width: 115px" @change="changeModel"
                                   :disabled="aiLoading">
                ...
                <div @click="onSubmit" class="sendIcon" v-show="!modelVisible && !aiLoading">
                    <el-icon :size="20">
                        <Promotion/>
                    </el-icon>
                </div>
                ...
                <div @click="onSubmit" class="sendIcon" style="background: #e752ac"
...
import {CopyDocument, Goods, Promotion, Star, UserFilled} from '@element-plus/icons-vue'
    ...
    setup() {
        ...
        function webSocket({messages, index}) {
            if (typeof (WebSocket) == "undefined") {
                ...
                let socketUrl = url.value + model.value + '/' + localStorage.getItem('token')
                ...
                socket.value.onmessage = function (msg) {
                    try {
                        if (msg.data === "SEND_ERR") {
                            socket.value.close();
                            conversationList.value[index].assistant = "哦豁貌似出了点小问题,请重新发送"
                            conversationList.value[index].isError = true
                            // eslint-disable-next-line no-empty
                        } else if (msg.data === "NO_FREQUENCY") {
                            socket.value.close();
                            conversationList.value[index].assistant = "当前可用次数不足以使用该功能,请前往个人中心充值"
                            conversationList.value[index].isError = true
                        } else {
                            let res = JSON.parse(msg.data);
                            // eslint-disable-next-line no-prototype-builtins
                            if (res.choices[0].hasOwnProperty('delta') && res.choices[0].delta.hasOwnProperty('content') && res.choices[0].delta.content !== "") {
                                messageQueue.push({
                ...
                socket.value.onerror = function () {
                    getUser();
                    conversationList.value[index].assistant = "与服务器建立连接失败，请联系在微信小程序中咨询客服解决问题"
                    conversationList.value[index].isError = true
...
>>> .answer > .el-avatar, >>> .question > .el-avatar {
